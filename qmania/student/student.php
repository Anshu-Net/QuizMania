...
<div class="container">
  ...
  <table class="table table-hover">
    ...
    <tbody>
      <?php
      $sql="SELECT * FROM `quiz` WHERE status='activated' AND `quizid` IN (SELECT quiz_id FROM `quizmap` WHERE `group_id`IN (SELECT `groupid` FROM `groupmap` WHERE `userid`='$student_id'))";
      $result=$conn->query($sql);
      while($row=$result->fetch_assoc()){
        ...
        echo "<tr>";
          ...
          if(status($quizid,$student_id,$conn)=='take'){
              echo "<td></td>";
              echo "<td><a href='quiz.php?id=".$row['quizid']."' class='btn btn-primary'>Take Quiz</a></td>";
          } 
